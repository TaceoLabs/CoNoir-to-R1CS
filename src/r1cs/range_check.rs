...
pub(crate) fn add_range_check_via_bit_decomposition<F: PrimeField>(
    r1cs: &mut NoirToR1CSCompiler<F>,
...
#[expect(unused)]
pub(crate) fn add_range_checks<F: PrimeField>(
    r1cs: &mut NoirToR1CSCompiler<F>,
    range_checks: BTreeMap<u32, Vec<usize>>,
) {
...
fn add_range_check_via_lookup<F: PrimeField>(
    r1cs_compiler: &mut NoirToR1CSCompiler<F>,
    num_bits: u32,
    ...
    let witness_summands = values_to_lookup
        .iter()
        .map(|value| {
            let witness_idx = add_lookup_factor(r1cs_compiler, sz_challenge, F::one(), *value);
    ...
    r1cs_compiler.r1cs.add_constraint(
        &[(F::one(), sum_for_table), (F::one().neg(), sum_for_witness)],
        &[(F::one(), r1cs_compiler.witness_one())],
        &[(F::zero(), r1cs_compiler.witness_one())],
    );
}
...
pub(crate) fn add_lookup_factor<F: PrimeField>(
    r1cs_compiler: &mut NoirToR1CSCompiler<F>,
    sz_challenge: usize,
    value_coeff: F,
    value_witness: usize,
    ...
    r1cs_compiler.r1cs.add_constraint(
        &[
            (F::one(), sz_challenge),
            (F::one().neg() * value_coeff, value_witness),
        ],
        &[(F::one(), r1cs_compiler.witness_one())],
        &[(F::one(), denominator)],
    );
...
fn add_naive_range_check<F: PrimeField>(
    r1cs_compiler: &mut NoirToR1CSCompiler<F>,
    num_bits: u32,
    index_witness: usize,
) {
    ...
    (1..(1 << num_bits) - 1).for_each(|index: u32| {
        let next_product_witness =
            r1cs_compiler.add_witness_builder(WitnessBuilder::ProductLinearOperation(
                r1cs_compiler.num_witnesses(),
                ProductLinearTerm(current_product_witness, F::one(), F::zero()),
                ProductLinearTerm(index_witness, F::one(), F::from(index).neg()),
            ));
        r1cs_compiler.r1cs.add_constraint(
            &[(F::one(), current_product_witness)],
            &[
                (F::one(), index_witness),
                (F::from(index).neg(), r1cs_compiler.witness_one()),
            ],
            &[(F::one(), next_product_witness)],
        );
