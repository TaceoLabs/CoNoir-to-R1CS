use crate::r1cs::binops::{BinOp, add_binop};
use crate::r1cs::memory::{MemoryBlock, MemoryOperation};
use crate::r1cs::r1cs_solver::{ConstantTerm, SumTerm, WitnessBuilder};
use crate::r1cs::ram::add_ram_checking;
use crate::r1cs::range_check::add_range_check_via_bit_decomposition;
use crate::r1cs::rom::add_rom_checking;
use crate::r1cs::{NoirElement, noir_to_native};
use eyre::bail;
use mpc_core::serde_compat::{ark_de, ark_se};
use {
    crate::r1cs::r1cs::R1CS,
    acir::{
        circuit::{
            Circuit, Opcode,
            opcodes::{
    ...
    std::{collections::BTreeMap, num::NonZeroU32},
};
...
pub(crate) struct NoirToR1CSCompiler<F: PrimeField> {
...
pub fn noir_to_r1cs<F: PrimeField>(
    circuit: &Circuit<NoirElement>,
) -> eyre::Result<(
    R1CS<F>,
    Vec<Option<NonZeroU32>>,
    Vec<WitnessBuilder<F>>,
    Vec<u32>,
)> {
...
impl<F: PrimeField> NoirToR1CSCompiler<F> {
    fn new() -> Self {
    ...
    // record this.
    pub fn add_witness_builder(&mut self, witness_builder: WitnessBuilder<F>) -> usize {
    ...
    pub(crate) fn add_sum(&mut self, summands: Vec<SumTerm<F>>) -> usize {
        let sum =
            self.add_witness_builder(WitnessBuilder::Sum(self.num_witnesses(), summands.clone()));
        let az = summands
            .iter()
            .map(|SumTerm(coeff, witness_idx)| (coeff.unwrap_or(F::one()), *witness_idx))
            .collect::<Vec<_>>();
        self.r1cs
            .add_constraint(&az, &[(F::one(), self.witness_one())], &[(F::one(), sum)]);
        sum
    }
    ...
    pub fn add_acir_assert_zero(&mut self, expr: &Expression<NoirElement>) {
        ...
        // their outputs
        let mut linear: Vec<(F, usize)> = vec![];
        let mut a: Vec<(F, usize)> = vec![];
        let mut b: Vec<(F, usize)> = vec![];
        ...
        if !expr.mul_terms.is_empty() {
            ...
            linear = expr
                ...
                .map(|(coeff, acir_witness_a, acir_witness_b)| {
                    ...
                    (-noir_to_native::<F>(*coeff), self.add_product(a, b))
                })
                .collect::<Vec<_>>();
            ...
            match opcode {
                ...
                Opcode::MemoryInit {
                    ...
                    assert!(
                        ...
                        "Memory block {block_id} already initialized"
                    );
                ...
                Opcode::MemoryOp {
                    ...
                    assert!(
                        memory_blocks.contains_key(&block_id),
                        "Memory block {block_id} not initialized before read"
                    );
...
pub enum ConstantOrR1CSWitness<F: PrimeField> {
    Constant(#[serde(serialize_with = "ark_se", deserialize_with = "ark_de")] F),
    Witness(usize),
}

impl<F: PrimeField> ConstantOrR1CSWitness<F> {
    pub fn to_tuple(&self) -> (F, usize) {
        match self {
            ConstantOrR1CSWitness::Constant(c) => (*c, WITNESS_ONE_IDX),
            ConstantOrR1CSWitness::Witness(w) => (F::one(), *w),
        }
    }
}
