use {
    crate::r1cs::{noir_to_r1cs::NoirToR1CSCompiler, r1cs_solver::WitnessBuilder},
...
/// codes).
pub(crate) fn add_digital_decomposition<F: PrimeField>(
    ...
    let mut digit_multipliers = vec![F::one()];
    for log_base in log_bases[..log_bases.len() - 1].iter() {
        let multiplier = *digit_multipliers.last().unwrap() * F::from(1u64 << *log_base);
        ...
        .for_each(|(i, value)| {
            ...
            digit_multipliers
                ...
                .for_each(|(digit_place, digit_multiplier)| {
                    ...
                    recomp_summands.push(((*digit_multiplier), digit_witness));
                });
...
pub(crate) fn decompose_into_digits<F: PrimeField>(value: F, log_bases: &[usize]) -> Vec<F> {
    let num_digits = log_bases.len();
    let mut digits = vec![F::zero(); num_digits];
...
pub(crate) fn field_to_le_bits<F: PrimeField>(value: F) -> Vec<bool> {
...
/// bits.
pub(crate) fn le_bits_to_field<F: PrimeField>(bits: &[bool]) -> F {
    ...
    let be_byte_vec: Vec<u8> = padded_bits_le
        ...
        .rev()
        .collect();
    F::from_be_bytes_mod_order(&be_byte_vec)
}
...
fn test_le_bits_to_field() {
    ...
    let value = le_bits_to_field::<FieldElement>(&bits);
