...
impl SpiceWitnesses {
    ...
    pub fn solve<F: PrimeField>(&self, witness: &mut [Option<F>]) {
        ...
        for (mem_op_index, mem_op) in self.memory_operations.iter().enumerate() {
            match mem_op {
                SpiceMemoryOperation::Load(addr, value, read_timestamp) => {
                    ...
                    let addr_as_usize = addr.into_bigint().as_ref()[0] as usize;
                    witness[*read_timestamp] = Some(F::from(rt_final[addr_as_usize] as u64));
        ...
        for i in 0..self.memory_length {
            ...
            witness[self.rt_final_start + i] = Some(F::from(rt_final[i] as u64));
        }
    }
}
...
pub fn add_ram_checking<F: PrimeField>(
    ...
    block
        ...
        .for_each(|(addr, mem_value)| {
            ...
            let factor = add_mem_op_multiset_factor(
                ...
                rs_challenge,
                rs_challenge_sqrd,
                (F::from(addr as u64), r1cs_compiler.witness_one()),
                *mem_value,
                (F::zero(), r1cs_compiler.witness_one()),
            );
            println!("WS factor [{factor}]: ({addr}, [{mem_value}], 0)");
        ...
        .for_each(|(mem_op_index, op)| {
            match op {
                SpiceMemoryOperation::Load(addr_witness, value_witness, rt_witness) => {
                    ...
                    println!(
                        "RS factor [{factor}]: ([{addr_witness}], [{value_witness}], [{rt_witness}])"
                    );
                ...
                SpiceMemoryOperation::Store(
                    ...
                    println!(
                        "RS factor [{factor}]: ([{addr_witness}], [{old_value_witness}], [{rt_witness}])"
                    );
                    rs_hash = r1cs_compiler.add_product(rs_hash, factor);
    ...
    all_mem_op_index_and_rt
        ...
        .for_each(|(mem_op_index, rt_witness)| {
            ...
            let difference_witness_idx = r1cs_compiler.add_sum(vec![
                SumTerm(
                    Some(F::from(*mem_op_index as u64)),
                    r1cs_compiler.witness_one(),
                ),
                SumTerm(Some(F::one().neg()), *rt_witness),
            ]);
...
// this.
fn add_mem_op_multiset_factor<F: PrimeField>(
    r1cs_compiler: &mut NoirToR1CSCompiler<F>,
    sz_challenge: usize,
    rs_challenge: usize,
    rs_challenge_sqrd: usize,
    (addr, addr_witness): (F, usize),
    value_witness: usize,
    (timer, timer_witness): (F, usize),
) -> usize {
    ...
    r1cs_compiler.r1cs.add_constraint(
        &[(F::one(), rs_challenge)],
        &[(F::one().neg(), value_witness)],
        &[
            (F::one(), factor),
            (F::one().neg(), sz_challenge),
            (timer, intermediate),
            (addr, addr_witness),
        ],
    );
