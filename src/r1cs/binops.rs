...
/// [BINOP_ATOMIC_BITS].
pub(crate) fn add_binop<F: PrimeField>(
    r1cs_compiler: &mut NoirToR1CSCompiler<F>,
    op: BinOp,
    inputs_and_outputs: Vec<(ConstantOrR1CSWitness<F>, ConstantOrR1CSWitness<F>, usize)>,
) {
    ...
    let summands_for_table = (0..1 << BINOP_ATOMIC_BITS)
        ...
        .map(|(lh_operand, rh_operand, output)| {
            let denominator = add_lookup_summand(
                ...
                rs_challenge,
                rs_challenge_sqrd,
                ConstantOrR1CSWitness::Constant(F::from(lh_operand)),
                ConstantOrR1CSWitness::Constant(F::from(rh_operand)),
                ConstantOrR1CSWitness::Constant(F::from(output)),
            );
...
fn add_lookup_summand<F: PrimeField>(
    r1cs_compiler: &mut NoirToR1CSCompiler<F>,
    sz_challenge: usize,
    rs_challenge: usize,
    rs_challenge_sqrd: usize,
    lh_operand: ConstantOrR1CSWitness<F>,
    rh_operand: ConstantOrR1CSWitness<F>,
    output: ConstantOrR1CSWitness<F>,
) -> usize {
    ...
    let rs_challenge_sqrd_summand = match output {
        ConstantOrR1CSWitness::Constant(value) => (value, rs_challenge_sqrd),
